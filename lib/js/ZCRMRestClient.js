...
ZCRMRestClient.initialize = function(configJSON, db){
    
	return new Promise(function(resolve,reject){

		if(configJSON){

			ZCRMRestClient.initializeWithValues(configJSON, db)
			resolve()

		}

		const resources = process.env.zohoVariable ? process.env.zohoVariable : 'resources/oauth_configuration.properties'
		...
		const dbModule = db === 'postgre' ? './postgre/model_util' : './mysql/mysql_util'
		db_module = config_properties.get('crm.api.tokenmanagement')?config_properties.get('crm.api.tokenmanagement'):dbModule
...
ZCRMRestClient.initializeWithValues = function(configJSON, db){
	...
	const dbModule = db === 'postgre' ? './postgre/model_util' : './mysql/mysql_util'
	db_module = configJSON.db_module?configJSON.db_module:dbModule
...
ZCRMRestClient.generateAuthTokens = function(user_identifier,grant_token){
	...
	return new Promise(function(resolve,reject){
		...
		OAuth.generateTokens(api_url).then(function(response){
			...
			var mysql_util = require(db_module)
